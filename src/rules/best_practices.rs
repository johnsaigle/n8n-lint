...
pub fn check_error_handling(nodes: &[Node]) -> Vec<Finding> {
    let has_error_trigger = nodes.iter().any(Node::is_error_trigger);
...
pub fn check_ntfy_presence(nodes: &[Node]) -> Vec<Finding> {
    ...
    let has_ntfy = nodes.iter().any(super::ntfy::is_ntfy_node);
...
pub fn check_manual_trigger(nodes: &[Node]) -> Vec<Finding> {
    let has_schedule = nodes.iter().any(Node::is_schedule_trigger);
    let has_manual = nodes.iter().any(Node::is_manual_trigger);
...
#[must_use]
pub const fn check_hardcoded_urls(_nodes: &[Node]) -> Vec<Finding> {
